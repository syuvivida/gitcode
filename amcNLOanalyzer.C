...
void amcNLOanalyzer(string inputName, string outputName){
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    TLorentzVector lepMinus(0,0,0,0);

    if (plusMax >= 0) 
      lepton.SetPtEtaPhiE(genParPt[plusMax], genParEta[plusMax], genParPhi[plusMax], genParE[plusMax]);
	...
	if (genParPt[ilep] > metPtmax) metPtmax = genParPt[ilep];
    ...
    for (Int_t i =0; i < nGenJet; i++){
      ...
      if (deltaR(LepEta_, LepPhi_, genJetEta[i], genJetPhi[i]) < 0.5) continue;
      if (genJetPt[i] < 30 || abs(genJetEta[i]) > 2.5 ) continue;

      njet++;

      if (genJetPt[i] > ptJetMax){
	ptJetMax = genJetPt[i];
	Jetmax = i;
      }
    ...
    if (Jetmax < 0) continue;

    Jet.SetPtEtaPhiE(genJetPt[Jetmax], genJetEta[Jetmax], genJetPhi[Jetmax], genJetE[Jetmax]);
    ...
    printProgBar(ev/(double)data.GetEntriesFast()*100);
