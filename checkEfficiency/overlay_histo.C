...
void overlay_histo(){
  ...
  /*
  string r_name[] = { "ckmu_hZ_qqll_LHC8_M1500.root",
		      "mu_hZ_qqll_LHC8_M1500.root" };
  ...
  */
  ...
  string r_name[] = { "ckel_hZ_qqll_LHC8_M1500.root",
		      "el_hZ_qqll_LHC8_M1500.root" };
  ...
  for(Int_t i=0; i<size_h; i++){ // loop over variables
    ...
    string print_name = "total_" + h_name[i] + ".eps";
    string line_name;
    Double_t get_max;
    Double_t temp = 0.0;
    ...
    for(Int_t j=0; j<size_r; j++){ // loop over masses again
      ...
      h[j]->GetYaxis()->SetTitleOffset(1.8);
      h[j]->SetLineWidth(2);
