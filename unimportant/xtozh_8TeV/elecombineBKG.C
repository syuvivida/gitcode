...
void elecombineBKG(){
  ...
  */
  ...
  TH1D* h_combineBKG = new TH1D("h_combineBKG", "Iso of Total Background", 100, 0, 50);
  ...
  Double_t bkgtotalPassEvent = h_combineBKG->Integral(0,101);
  Double_t sigtotalPassEvent = h_sgn->Integral(0,101);
  ...
  for(Int_t bin = 0; bin < 51; bin++){

    upperThreshold.push_back(bin*(50./100));
  ...
  h_bkgtotalIsoEff->SetMaximum(1);
  h_bkgtotalIsoEff->SetMinimum(0);
