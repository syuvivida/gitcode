...
void combineBKG(){
  ...
  Double_t bkgtotalPassEvent = h_combineBKG->Integral(0,15);
  Double_t sigtotalPassEvent = h_sgn->Integral(0,15);
  ...
  for(Int_t bin = 0; bin <= 15; bin++){
    ...
    bkgisolationEff.push_back( B[bin] / bkgtotalPassEvent );
    sigisolationEff.push_back( S[bin] / sigtotalPassEvent );
