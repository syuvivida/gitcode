...
void passMuonID(TreeReader &data, 
  ...
  for(mapMuIter it_part = sortMuPt.begin(); it_part != sortMuPt.end(); ++it_part){
    ...
    // if( (muTauCorrPfIso[sortMuIndex] / muPt[sortMuIndex]) >= 0.1 ) continue;
  ...
  if( howManyMu.size() < 2 ) return;

  if( onlyGlobalIndex.size() < 1 && 
      onlyTightPtIdex.size() < 1 && 
      globalTightPtIndex.size() < 1 ) return;

  if( globalTightPtIndex.size() < 1 && 
      onlyGlobalIndex.size() < 1 ) return;

  if( globalTightPtIndex.size() < 1 && 
      onlyTightPtIdex.size() < 1 ) return;
