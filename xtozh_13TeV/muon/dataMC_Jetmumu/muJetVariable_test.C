...
void muJetVariable_test(std::string inputFile, std::string outputFile){
  ...
  TH1D* h_FATjetPRmass     = new TH1D("h_FATjetPRmass", "FATjetPRmass",  nvarBins, varBins);
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    h_FATjetPRmass    ->Fill((*thisMu+*thatMu+*thisJet).M(),eventWeight);
