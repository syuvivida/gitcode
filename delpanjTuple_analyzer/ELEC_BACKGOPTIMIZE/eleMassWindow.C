...
void eleMassWindow(std::string inputFile, std::string outName, 
  ...
  TH1D* h_TrkIso = new TH1D("h_TrkIso", "Iso", 100, 0, 0.5);
  h_TrkIso->GetXaxis()->SetTitle("Iso");
  h_TrkIso->GetYaxis()->SetTitle("Event number");

  TH1D* h_CalIso = new TH1D("h_CalIso", "Iso", 100, 0, 0.5);
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	h_TrkIso->Fill(eleUserTrkIso[stRecoEleIndex]);
	h_TrkIso->Fill(eleUserTrkIso[ndRecoEleIndex]);

	h_CalIso->Fill(eleUserCalIso[stRecoEleIndex] - (0.06205 * eleRho));
	h_CalIso->Fill(eleUserCalIso[ndRecoEleIndex] - (0.06205 * eleRho));
