...
// root -q -b eleVariable.C++\(\"/data7/khurana/NCUGlobalTuples/Run2015C/SingleElectron_Run2015C-PromptReco-v1/0000\"\,0\)
// root -q -b eleVariable.C++\(\"/data7/khurana/NCUGlobalTuples/SPRING15/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_25ns/crab_DYJetsToLL_M-50_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_0830/150830_215828/0000\"\,1\)
// root -q -b eleVariable.C++\(\"/data7/khurana/NCUGlobalTuples/SPRING15/TT_TuneCUETP8M1_13TeV-powheg-pythia8_0803/150803_175618/0000\"\,2\)
...
void eleVariable(std::string inputFile, int num){
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	Float_t ptmin = TMath::Min(pt1,pt2);
	...
	for(Int_t flag = 0; flag <= 7; flag++){
	  ...
	  if( eleSigmaIEtaIEtaFull5x5[eleId[ie]] >= 999       && flag != 3 ) continue; 
	  if( eleMissHits[eleId[ie]]             >  1         && flag != 4 ) continue;
	  if( fabs(eleD0[eleId[ie]])             >= 0.02      && flag != 5 ) continue;
	  if( eleMiniIso[eleId[ie]]              >= 0.1       && flag != 6 ) continue;
	  if( (eleE2x5Full5x5[eleId[ie]]/eleE5x5Full5x5[eleId[ie]] <= 0.94  &&
	       eleE1x5Full5x5[eleId[ie]]/eleE5x5Full5x5[eleId[ie]] <= 0.83) && flag != 7 )
	    continue;
	...
	for(Int_t flag = 0; flag <= 7; flag++){
	  ...
	  if( eleMiniIso[eleId[ie]]              >= 0.1      && flag != 6 ) continue;
	  if( (eleE2x5Full5x5[eleId[ie]]/eleE5x5Full5x5[eleId[ie]] <= -999  &&
	       eleE1x5Full5x5[eleId[ie]]/eleE5x5Full5x5[eleId[ie]] <= -999) && flag != 7 )
	    continue;
	  	    
	  switch(flag){

	  case 0: h_eleEtaseedAtVtx[1]->Fill(eleEtaseedAtVtx[eleId[ie]]); break;
	  case 1: h_eledPhiAtVtx[1]   ->Fill(eledPhiAtVtx[eleId[ie]]);    break;
	  case 2: h_eleHoverE[1]      ->Fill(eleHoverE[eleId[ie]]);       break;
	  case 3: h_eleSigmaIEtaIEtaFull5x5[1]->Fill(eleSigmaIEtaIEtaFull5x5[eleId[ie]]); break;
	  case 4: h_eleMissHits[1]    ->Fill(eleMissHits[eleId[ie]]);     break;
	  case 5: h_eleD0[1]          ->Fill(eleD0[eleId[ie]]);           break;
	  case 6: h_eleMiniIso[1]     ->Fill(eleMiniIso[eleId[ie]]);      break;	  
	  case 7: {
	    h_eleFull5x5E2x5dvE5x5[1] ->Fill(eleE2x5Full5x5[eleId[ie]]/eleE5x5Full5x5[eleId[ie]]);
	    h_eleFull5x5E1x5dvE5x5[1] ->Fill(eleE1x5Full5x5[eleId[ie]]/eleE5x5Full5x5[eleId[ie]]);
	  } break;
  ...
  std::string h_name[9] = {"eleEtaseedAtVtx","eledPhiAtVtx","eleHoverE","eleSigmaIEtaIEtaFull5x5",
